...
func TestDecodeModule(t *testing.T) {
	...
	for _, tt := range tests {
		...
		t.Run(tc.name, func(t *testing.T) {
			m, e := DecodeModule(binaryencoding.EncodeModule(tc.input), api.CoreFeaturesV1, wasm.MemoryLimitPages, false, false, false, true)
			require.NoError(t, e)
	...
	t.Run("skips custom section", func(t *testing.T) {
		input := append(append(Magic, version...),
			...
			0x04, 'm', 'e', 'm', 'e',
			1, 2, 3, 4, 5, 6, 7, 8, 9, 0)
		m, e := DecodeModule(input, api.CoreFeaturesV1, wasm.MemoryLimitPages, false, false, false, true)
		require.NoError(t, e)
	...
	t.Run("reads custom sections", func(t *testing.T) {
		input := append(append(Magic, version...),
			...
			0x04, 'm', 'e', 'm', 'e',
			1, 2, 3, 4, 5, 6, 7, 8, 9, 0)
		m, e := DecodeModule(input, api.CoreFeaturesV2, wasm.MemoryLimitPages, false, false, true, true)
		require.NoError(t, e)
	...
	t.Run("DWARF enabled", func(t *testing.T) {
		m, err := DecodeModule(dwarftestdata.ZigWasm, api.CoreFeaturesV2, wasm.MemoryLimitPages, false, true, true, true)
		require.NoError(t, err)
		require.NotNil(t, m.DWARFLines)
	})

	t.Run("DWARF disabled", func(t *testing.T) {
		m, err := DecodeModule(dwarftestdata.ZigWasm, api.CoreFeaturesV2, wasm.MemoryLimitPages, false, false, true, true)
		require.NoError(t, err)
	...
	t.Run("data count section disabled", func(t *testing.T) {
		...
		_, e := DecodeModule(input, api.CoreFeaturesV1, wasm.MemoryLimitPages, false, false, false, true)
...
func TestDecodeModule_Errors(t *testing.T) {
	...
	for _, tt := range tests {
		...
		t.Run(tc.name, func(t *testing.T) {
			_, e := DecodeModule(tc.input, api.CoreFeaturesV1, wasm.MemoryLimitPages, false, false, false, true)
